class FlaggedFeedbacksController < ApplicationController
  ...
  def index
    ...
    if params[:commit] == 'Clear'
      ...
      @pagy, @flagged_feedbacks = pagy(Feedback.joins(:user).select("feedbacks.id, feedbacks.user_id, users.username, feedbacks.comment, feedbacks.status, feedbacks.category, feedbacks.active_status, feedbacks.created_at").order('created_at DESC').search_dates(session[:admin_flagged_feedback_start_date], session[:admin_flagged_feedback_end_date]).where.not(active_status: 0), items: 10, size: [1,0,0,1])
    else
      self.set_submit_fields('attribute', @search_page)
      @pagy, @flagged_feedbacks = pagy(Feedback.joins(:user).select("feedbacks.id, feedbacks.user_id, users.username, feedbacks.comment, feedbacks.status, feedbacks.category, feedbacks.active_status, feedbacks.created_at").order('created_at DESC').search(session[:admin_flagged_feedback_search_type], session[:admin_flagged_feedback_search_term]).where.not(active_status: 0), items: 10, size: [1,0,0,1])
    end
