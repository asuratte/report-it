class KpiDashboardController < ApplicationController
    def index
        if params[:commit] == 'Choose Dates' && params[:start_date].present? && params[:end_date].present? && params[:start_date] <= params[:end_date]
            ...
            @new_users = User.all.where(created_at: (start_date_beginning_of_day)..(end_date_end_of_day))
            @deactivated_users = User.all.where(deactivated_at: (start_date_beginning_of_day)..(end_date_end_of_day))
            @invalid_date = false
