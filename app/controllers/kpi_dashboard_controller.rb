class KpiDashboardController < ApplicationController
    def index
        if params[:commit] == 'Choose Dates' && params[:start_date].present? && params[:end_date].present? && params[:start_date] <= params[:end_date]
            ...
            @deactivated_reports = Report.where(deactivated_at: (start_date_beginning_of_day)..(end_date_end_of_day)).where.not(deactivated_at: nil).where.not(active_status: 'active')
            @new_users = User.all.where(role: 'resident').where(created_at: (start_date_beginning_of_day)..(end_date_end_of_day))
            @deactivated_users = User.all.where(role: 'resident').where.not(deactivated_at: nil).where(deactivated_at: (start_date_beginning_of_day)..(end_date_end_of_day)).where(active: false)
            ...
            @deactivated_reports = Report.all.where.not(deactivated_at: nil).where.not(active_status: 'active')
            @new_users = User.all.where(role: 'resident')
            @deactivated_users = User.all.where(role: 'resident').where.not(deactivated_at: nil).where(active: false)
            @all_time = true
